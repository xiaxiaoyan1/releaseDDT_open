...
from common.email_report import Email
...
def Run(configFile,Sheet):
    try:
        ...
        reportpath = Write().WriteNewfile(run_project,sheet)#创建报告表
        ...
        # with open("/www/wwwroot/47.93.62.235/TV3.0/TV3.0-Report%s.html"%(reportTime),'w') as ch:
        #     ch.write(content)
        # print("http://47.93.62.235/TV3.0/TV3.0-Report%s.html" % (reportTime))
        if failnum == 0 and sendtime == "1000" or failnum == 0 and sendtime == "1800":
            with open("/www/wwwroot/47.93.62.235/%s/%s-Report%s.html" % (run_project,sheet,reportTime),'w') as ch:
                ch.write(content)
            print("http://47.93.62.235/TV3.0/TV3.0-Report%s.html" % (reportTime))
            get_dingtalk.sendapi("%s-定时运行结果:"%(run_project)+
                              "\n"+"成功：%s"%(succenum)+
                              "\n"+"失败：%s"%(failnum)+
